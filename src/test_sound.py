...
class SoundThread(QThread):
    """ 사운드 재생을 위한 QThread """
    update_sound = pyqtSignal(float, float, bool)  # a, b, switch 상태를 업데이트하는 시그널

    def __init__(self):
        super().__init__()
        self.a = 100  # 기본 주파수 조절 값
        self.b = 0.5  # 좌우 볼륨 비율
        self.switch_on = True  # 스위치 상태 (True: 재생, False: 정지)
    ...
    # """ QThread에서 실행될 내용 """
        def callback(outdata, frames, time, status):
            ...
            if self.switch_on:
                t = (np.arange(frames) + callback.time) / 44100* self.a /100 # 시간 벡터 계산
